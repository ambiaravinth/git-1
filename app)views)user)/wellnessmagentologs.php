...
<script>
    var tableUser = $('#table-reg_service_data').DataTable({
        ...
        ajax: {
            url: '<?= route_to('admin/user/wellness-magento-logs') ?>',
            method: 'GET',
            data: function(data) {
                // Read values
                var min = $('#min').val();
        ...
        columns: [{
            ...
            },
            {
                "data": function(data) {
                    return `<td class="text-right py-0 align-middle">
                            <div class="btn-group btn-group-sm">
                                <a href="<?= route_to('admin/user/wellness-magento-logs') ?>/${data.reg_id}/edit" target="_blank" class="btn btn-primary btn-edit"><i class="far fa-eye"></i></a>
    ...
    $(document).on('click', '.btn-delete', function(e) {
        Swal.fire({
            ...
            .then((result) => {
                if (result.value) {
                    $.ajax({
                        url: `<?= route_to('admin/user/wellness-magento-logs') ?>/${$(this).attr('data-id')}`,
                        method: 'DELETE',
