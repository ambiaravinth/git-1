...
class WellnessMagentoLogsModel extends Model
{
    protected $table = 'wellness_magento_logs';
    ...
    {
        $builder = $this->builder()->select('reg_id,lab_id,testgroupcode,Attachment,status,response,created_at');
        $condition = empty($search) ? $builder : $builder->groupStart()->like('lab_id', $search)->orLike('testgroupcode', $search)->groupEnd();
